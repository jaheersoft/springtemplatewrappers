...
public abstract class AbstractRESTPOSTClient<R> {
	...
	public AbstractRESTPOSTClient(RESTServiceCaller caller) {
		this.caller = caller;
	}
	
	public AbstractRESTPOSTClient<R> withExternalAPIURI(String externalAPIURI) {
		this.externalAPIURI = externalAPIURI;
		return this;
	}
	
	public AbstractRESTPOSTClient<R> from(Inputs inputs) {
		this.inputs = inputs;
		return this;
	}
	
	public AbstractRESTPOSTClient<R> forResponseClass(Class<R> responseClass) {
	...
	public <Q> AbstractRESTPOSTClient<R> call() throws ApplicationException,FaultError {
		Q request = null;
		try {
		...
		try {
			...
			throw new ApplicationException.Builder(e).the(REST_ERROR).occurredWhile(BUILDING_REQUEST).build();
		}
